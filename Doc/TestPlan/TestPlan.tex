...
the mathematical range of a user-defined function given the domains of
the function's component variables. Function ranges and variable
...
\begin{itemize}
	...
	The selected version of Visual Studio allows developers to record a series 
	of interactions with a GUI that can be played multiple times for testing. 
	This can help save time by making traditionally manual test cases into 
	automated ones. Depending on the complexity of the GUI, this tool might not 
	be used.
...
\begin{enumerate}
	...
	Output: Warning -- Domain for variable $y$ is ordered highest to lowest; 
	reordering values to be smallest to highest\\
	\Tree[.$+$ [.$x$ $[2,4]$ ] [.$y$ $[-5,3]$ ] ]
...
\begin{enumerate}
	...
	\item{test-parse\_divisionMixedIntervalDivisorZero1}
	...
	\item{test-parse\_divisionMixedIntervalDivisorZero2}
...
\rref{R_CalculateCompose} \wss{Very nice to have explicit
  cross-references between documents.  To keep the information up to
  date, consider using make to build your documentation.}
(Recomposing the results from the component equations of $f(V)$) and
\rref{R_VerifyOutput} (Ensuring that the calculated result is
...
\begin{enumerate}
	...
	Output: True \wss{What does True mean? $R(f)$ does not have
          the type Boolean?}
...
\wss{Great to see usability testing!  Could you please give some more
  detail on how the tests will be run.  How many participants?  How
  will they be recruited?  What documentation will they be given?
  etc.  What specific task or tasks will they be asked to perform?
  How will the results of the usability tests be summarized?}
...
and graph (Figure~\ref{Fig:req2test}) \wss{Label and reference do not
  match} is to provide easy references between the test suite and the
requirements from the SRS (version \srsVersion{}). In the table, the
test suites that verify a requirement are marked with an ``X'' in the
requirement's column. In the graph, tests appear at the tail of an
...
determine its usefulness and accessibility to the rest of the
program.  \wss{You should explicitly state that your goal is 100\%
  code coverage.}
