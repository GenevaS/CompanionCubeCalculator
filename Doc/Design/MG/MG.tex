...
\item[\refstepcounter{acnum} \actheacnum \label{acCalculations}:] How the 
calculation of mathematical operators on intervals is defined based on the 
input data.

\item[\refstepcounter{acnum} \actheacnum \label{acCalculationAlgorithm}:] The 
algorithm used to calculate $R(f(V))$.
...
\item[\refstepcounter{ucnum} \uctheucnum \label{ucDecomposing}:] The allowable 
operators in the user inputs are supported by the decomposition algorithm.

\item[\refstepcounter{ucnum} \uctheucnum \label{ucCalculating}:] The 
allowable operators in the user inputs are supported by the calculation 
algorithm.
