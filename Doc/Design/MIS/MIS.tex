...
\begin{center}
	\begin{tabular}{p{3.2cm} p{2.8cm} p{3cm} p{5cm}}
		...
		\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
		\hline
		\multirow{5}{3.2cm}{MakeEquationTree} & \multirow{5}{2.8cm}{$String$, 
		$String^n$} & \multirow{5}{3cm}{$equationStruct$} & 
		EQC\_INSUFF\_PARAMS, \\
...
\begin{itemize}
	...
	|\\
	(userEquation == ``" \Rightarrow EQC\_NO\_FUNC)\\
	|\\
...
\begin{center}
	\begin{tabular}{p{3cm} p{3cm} p{3cm} p{5cm}}
		...
		\multirow{3}{3cm}{SetMinBound} & \multirow{3}{3cm}{$\mathbb{R}$} & 
		\multirow{3}{3cm}{-} & IV\_NOT\_REAL\_TYPE, \\
		 &  &  & IV\_INSUFF\_PARAMS, \\
		  &  &  & ORD\_VIOLATED \\
		\multirow{3}{3cm}{SetMaxBound} & \multirow{3}{3cm}{$\mathbb{R}$} & 
		\multirow{3}{3cm}{-} & IV\_NOT\_REAL\_TYPE, \\
		 &  &  & IV\_INSUFF\_PARAMS, \\
		   &  &  & ORD\_VIOLATED \\
		\hline
...
\begin{itemize}
	...
	\item exception: $exc := \\
	(minB \notin \mathbb{R} \vee maxB \notin \mathbb{R} \Rightarrow 
	IV\_NOT\_REAL\_TYPE) \\
	|\\
	(\nexists minB \vee \nexists maxB \Rightarrow IV\_INSUFF\_PARAMS)\\
	|\\
	(minB > maxB \Rightarrow ORD\_VIOLATED)$
\end{itemize}
...
\begin{itemize}
	...
	|\\
	(\nexists minB \Rightarrow IV\_INSUFF\_PARAMS) \\
	|\\
	(minB > maxBound \Rightarrow ORD\_VIOLATED)$
\end{itemize}
...
\begin{itemize}
	...
	(\nexists maxB \Rightarrow IV\_INSUFF\_PARAMS) \\
	|\\
	(maxB < a \Rightarrow ORD\_VIOLATED)$
\end{itemize}
...
\begin{longtable}{l p{9.5cm}}
	...
	EQC\_NO\_FUNC & Error: No user equation was passed to the equation 
	conversion process. \\
