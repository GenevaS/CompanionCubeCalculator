...
\begin{center}
	\begin{tabular}{p{3cm} p{3cm} p{3cm} p{5cm}}
		...
		\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
		\hline
		\multirow{2}{3cm}{OperatorStruct} & $String$, $Int$, $Boolean^4$ & 
		\multirow{2}{3cm}{$operatorStruct$} & 
		\multirow{2}{5cm}{OP\_MISSING\_OP, OP\_MULTI\_TYPE}\\
		GetOperator & - & $String$ & - \\
		GetPrecedence & - & $Int$ & - \\
		IsUnary & - & $Boolean$ & - \\
		IsBinary & - & $Boolean$ & - \\
...
\begin{itemize}
	...
	\item transition: Update state variables $operator$, $precedence$, and $leftAssociative$ with the provided values $op$, $prec$, and $isLeftAssociative$. Set the value of $numOperands$ as:
	\begin{itemize} 
		\item $1$ if $isUnary = True$
		\item $2$ if $isBinary = True$
		\item $3$ if $isTernary = True$
	\end{itemize}
	\item exception: $exc := \\
	(op = ``" \Rightarrow OP\_MISSING\_OP)\\
	|\\
	((isUnary = True \wedge isUnary = isBinary) \vee (isUnary = True \wedge isUnary = isTernary) \vee (isBinary = True \wedge isBinary = isTernary) \Rightarrow OP\_MULTI\_TYPE)$
\end{itemize}
...
\begin{longtable}{l p{9.5cm}}
	...
	OP\_MULTI\_TYPE & Error: An operator cannot be overloaded to be unary, binary, and ternary. \\
